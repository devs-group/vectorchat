...
func (r *ChatbotRepository) Create(ctx context.Context, chatbot *Chatbot) error {
	...
	query := `
		INSERT INTO chatbots (
			...
			model_name, temperature_param, max_tokens, is_enabled, created_at, updated_at
		) VALUES (
			:id, :user_id, :name, :description, :system_instructions,
			:model_name, :temperature_param, :max_tokens, :is_enabled, :created_at, :updated_at
		)
...
func (r *ChatbotRepository) FindByIDAndUserID(ctx context.Context, id uuid.UUID, userID string) (*Chatbot, error) {
	...
	err := r.db.GetContext(ctx, &chatbot, query, id, userID)
	if err != nil {
		if IsNoRowsError(err) {
			return nil, apperrors.ErrChatbotNotFound
		}
...
func (r *ChatbotRepository) Update(ctx context.Context, chatbot *Chatbot) error {
	...
	query := `
		...
		SET name = :name, description = :description, system_instructions = :system_instructions,
		    ...
		    max_tokens = :max_tokens, is_enabled = :is_enabled, updated_at = :updated_at
...
func (r *ChatbotRepository) UpdateBasicInfo(ctx context.Context, id uuid.UUID, userID, name, description string) error {
	...
	result, err := r.db.ExecContext(ctx, query, name, description, time.Now(), id, userID)
	if err != nil {
...
func (r *ChatbotRepository) UpdateSystemInstructions(ctx context.Context, id uuid.UUID, userID, instructions string) error {
	...
	result, err := r.db.ExecContext(ctx, query, instructions, time.Now(), id, userID)
	if err != nil {
...
func (r *ChatbotRepository) UpdateModelSettings(ctx context.Context, id uuid.UUID, userID, modelName string, temperature float64, maxTokens int) error {
	...
	result, err := r.db.ExecContext(ctx, query, modelName, temperature, maxTokens, time.Now(), id, userID)
	if err != nil {
